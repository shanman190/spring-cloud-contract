...
configure(listOf(project(":fraudDetectionService"), project(":loanApplicationService"))) {
	...
	val contractsDir = file("mappings")
	val stubsOutputDirRoot = file("${project.buildDir}/production/${project.name}-stubs/")
	...
	configure<ContractVerifierExtension> {
		...
		contractsDslDir = file("${project.projectDir}/mappings/")
